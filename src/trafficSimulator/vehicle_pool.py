...
class VehiclePool:
    ...
    def vehicle_generator(self):
        if self.sim.t - self.last_added_time >= 60 / self.vehicle_rate:
            ...
            vehicle = Vehicle({'id': self.vehicle_count, 'have_traffic_signal': self.sim.have_traffic_signal},
                              self.v_max)
    ...
    def update(self, dt):
        ...
        for vehicle in self.vehicle_pool_list:
            ...
            if (vehicle.already_in_buffer is False) & (vehicle.is_in_buffer is True):
                ...
                vehicle.time_reach_buffer = self.sim.t + self.latency
                vehicle.time_reach_collision = self.sim.t + 60 + self.latency
                vehicle.time_out_collision = self.sim.t + 200 + self.latency
            # out of buffer
            elif (vehicle.already_in_buffer is True) & (vehicle.is_in_buffer is False):
            ...
            elif (vehicle.already_in_buffer is True) & (vehicle.is_in_buffer is True):
                # update priority
                if vehicle.from_which_direction == 'WEST':
            ...
            if (vehicle.already_in_collision is False) & (vehicle.is_in_collision is True):
            ...
            elif (vehicle.already_in_collision is True) & (vehicle.is_in_collision is False):
            ...
            if self.sim.have_traffic_signal:
                ...
                if signal_state:
                    ...
                    if vehicle.current_road_index == 0:
                        ...
                        if (vehicle_index == 0 and len(self.vehicles_in_collision) <= 1 and (
                                self.sim.t - self.last_pass_time > 10)):
                            vehicle.unstop()
                            vehicle.unslow()
                        elif (self.vehicles_in_buffer[0].stopped is True) and (lead is None) and (
                                vehicle.current_road_index > 0) and (self.sim.t - self.last_pass_time > 6):
                            self.vehicles_in_collision.append(vehicle)
                            self.last_pass_time = self.sim.t
                            # self.vehicles_in_buffer.remove(vehicle)
                            vehicle.unstop()
                    ...
                    elif (vehicle.already_in_collision is True) or (vehicle.already_out_collision is True):
                        vehicle.unstop()
                        vehicle.unslow()
                    else:
                        # bug
